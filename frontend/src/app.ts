...
async function createGame(): Promise<void> {
    const button = document.getElementById('create-game-btn') as HTMLButtonElement;
    if (!button) return;
    ...
    try {
        ...
        navigateToGame(result.game.code);
    } catch (error) {
...
async function joinGame(): Promise<void> {
    ...
    const button = document.getElementById('join-game-btn') as HTMLButtonElement;
    if (!button) return;

    try {
        // Set loading state
        setJoinButtonLoading(button, true);
        ...
        // Remove loading state
        setJoinButtonLoading(button, false);
    }
}

// Button loading state management with fixed dimensions
function setButtonLoading(button: HTMLButtonElement, isLoading: boolean): void {
    if (isLoading) {
        // Capture original dimensions to prevent layout shift
...
function setJoinButtonLoading(button: HTMLButtonElement, isLoading: boolean): void {
    if (isLoading) {
        // Capture original dimensions to prevent layout shift
        const originalWidth = button.offsetWidth;
        const originalHeight = button.offsetHeight;
        ...
        button.disabled = false;
        button.textContent = button.dataset.originalText || 'Join Game';
...
function navigateToGame(gameCode: string): void {
    // Update the browser URL
    const newUrl = `/games/${gameCode}`;
    window.history.pushState({ gameCode }, `Chess Doubles - Game ${gameCode}`, newUrl);

    // You could also dispatch a custom event for more complex routing
    document.dispatchEvent(new CustomEvent('app:navigate', {
        detail: { path: newUrl, gameCode }
    }));

    addMessage({ info: `Navigated to game ${gameCode}` });
}
